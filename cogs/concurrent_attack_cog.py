...
class TargetDamageModal(Modal):
    def __init__(self, boss_info: mongo.BossInfo, attack_kind: AttackKind) -> None:
        super().__init__(title=f"{boss_info.number}ボス: {attack_kind.Label()} 目標ダメージ入力")
    ...
    async def callback(self, interaction: discord.Interaction):
        damage = int(self.children[0].value)
        memo = self.children[1].value

        reserve = mongo.ConcurrentAttackReserve(
            guild_id=interaction.guild_id,
            user_id=interaction.user.id,
            boss_number=self.boss_info.number,
            kind=self.attack_kind.value,
            damage=damage,
            memo=memo,
        )
        reserve.Set()

        reserve_list: list[mongo.ConcurrentAttackReserve] = mongo.ConcurrentAttackReserve.Gets(
            interaction.guild.id, self.boss_info.number
        )
        embed = make_reserve_embed(interaction.guild, self.boss_info, reserve_list)
...
class AttackStartModal(Modal):
    def __init__(
        self, guild_id: int, members: list[discord.Member], boss_info: mongo.BossInfo, message_url: str
    ) -> None:
        super().__init__(title=f"{boss_info.number}ボス {boss_info.name} 同時凸開始")
        self.boss_info = boss_info
        self.message_url = message_url
        self.mentions = " ".join([mem.mention for mem in members])
        temp_msg = mongo.TemplateAttackStartMessage.Get(guild_id=guild_id, boss_number=boss_info.number)
        if temp_msg:
            temp = Template(temp_msg.template)
            val_map = {"boss_number": boss_info.number, "boss_name": boss_info.name}
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if guild is not None:
            event_title = f"{self.boss_info.number}ボス 同時凸中"
...
class ConcurrentAttackReserveView(View):
    def __init__(self, boss_info: mongo.BossInfo):
        # making None is important if you want the button work after restart!
        super().__init__(timeout=None)
        self.lock_lock = asyncio.Lock()
        self.replace_locks = {}
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(
                label=AttackKind.Physical.Label(), emoji=AttackKind.Physical.Emoji(), value=AttackKind.Physical.name
            ),
            discord.SelectOption(
                label=AttackKind.Magic.Label(), emoji=AttackKind.Magic.Emoji(), value=AttackKind.Magic.name
            ),
            discord.SelectOption(
                label=AttackKind.CarryOver.Label(), emoji=AttackKind.CarryOver.Emoji(), value=AttackKind.CarryOver.name
            ),
        ],
    )
    async def AttackSelectCallback(self, select: Select, interaction: discord.Interaction):
        attack_kind = AttackKind[select.values[0]]
        modal = TargetDamageModal(self.boss_info, attack_kind)
        await interaction.response.send_modal(modal=modal)
    ...
    async def CancelAttackButton(self, button, interaction: discord.Interaction):
        ...
        target_reserve = next(filter(lambda r: r.user_id == interaction.user.id, reversed(reserve_list)), None)
        if target_reserve is not None:
            target_reserve.Delete()
            reserve_list.remove(target_reserve)
        embed = make_reserve_embed(interaction.guild, self.boss_info, reserve_list)
        await interaction.response.edit_message(embed=embed)
    ...
    @btn_log.log("push attack start button")
    async def AttackStartButton(self, button, interaction: discord.Interaction):
        members = get_attack_members(interaction.guild)
        boss_content = atk_list[0].split(" ")[0]
        boss_number = int(boss_content.split(":")[0])
        boss_name = boss_content.split(":")[1]
        modal = AttackStartModal(
...
class ConcurrentAttackCog(commands.Cog):
    ...
    @slash_command(guild_ids=config.guild_ids, name="concurrent_atk", description="同時凸予約表を作成する")
    ...
    async def ConcurrentAttackCommand(
        ...
        boss_info = mongo.BossInfo.Get(boss_num)
        if boss_info is None:
            return await ctx.respond(f"{boss_num}ボス情報が登録されていません。", ephemeral=True)
        reserve_list: list[mongo.ConcurrentAttackReserve] = mongo.ConcurrentAttackReserve.Gets(
            ctx.guild.id, boss_info.number
        )

        view = ConcurrentAttackReserveView(boss_info)
        embed = make_reserve_embed(ctx.guild, boss_info, reserve_list)
...
def make_reserve_embed(
    guild: discord.Guild, boss_info: mongo.BossInfo, reserve_list: list[mongo.ConcurrentAttackReserve]
):
    embed = discord.Embed(title=f"{boss_info.number}ボス:{boss_info.name}, 最大HP:{boss_info.hp}万")

    p_content, m_content, c_content = make_reserve_content(guild, reserve_list)
    embed = (
        embed.add_field(
            name=f"{AttackKind.Physical.Emoji()} {AttackKind.Physical.Label()}",
            value=p_content,
            inline=False,
        )
        .add_field(
            name=f"{AttackKind.Magic.Emoji()} {AttackKind.Magic.Label()}",
            value=m_content,
            inline=False,
        )
        .add_field(
            name=f"{AttackKind.CarryOver.Emoji()} {AttackKind.CarryOver.Label()}",
            value=c_content,
            inline=False,
        )
    )
...
def make_reserve_content(guild: discord.Guild, reserve_list: list[mongo.ConcurrentAttackReserve]):
    physical_reserves = [r for r in reserve_list if r.kind == AttackKind.Physical.value]
    physical_content = "\n".join(
        [f"{guild.get_member(r.user_id).display_name}: {r.damage}万, {r.memo}" for r in physical_reserves]
    )
    magic_reserves = [r for r in reserve_list if r.kind == AttackKind.Magic.value]
    magic_content = "\n".join(
        [f"{guild.get_member(r.user_id).display_name}: {r.damage}万, {r.memo}" for r in magic_reserves]
    )
    carry_over_reserves = [r for r in reserve_list if r.kind == AttackKind.CarryOver.value]
    carry_over_content = "\n".join(
        [f"{guild.get_member(r.user_id).display_name}: {r.damage}万, {r.memo}" for r in carry_over_reserves]
    )
    return physical_content, magic_content, carry_over_content
