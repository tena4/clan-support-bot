...
def scraping(nowdate: date) -> Optional[list[bossMap]]:
    ...
    try:
        ...
        for i in range(1, 6):
            boss_name_tag = soup.find(
                "a", {"class": "anchor_super", "name": f"boss_{i}"}
            )
            boss_name = boss_name_tag.parent.get_text().strip().split()[0]
            pre_hp_tag = boss_name_tag.find_next(
                "img", {"alt": "4段階目", "title": "4段階目"}
            )
            hp = int(
                pre_hp_tag.parent.next_sibling.next_sibling.get_text().replace(",", "")
            )
        ...
        logger.error(f"scriping HTTP error occurred. {e}")
        return
